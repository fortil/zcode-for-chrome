...
// Chromium's History Manipulation Intervention marks entries added without
// user activation as skippable, so chrome.tabs.goBack/goForward fail with
// this message even though the entry exists. history.back()/forward() from
// the renderer does not apply the skipping, hence the fallback below.
const NOT_FOUND_NEXT_PAGE_ERROR = "Cannot find a next page in history";
const HISTORY_FALLBACK_POLL_MS = 150;
const HISTORY_FALLBACK_TIMEOUT_MS = 3000;

// Self-contained: injected via executeScript, which serializes it with
// toString(), so it cannot reference anything outside its own body.
function historyTraverse(arg: { direction: "back" | "forward" }): void {
  if (arg.direction === "back") history.back();
  else history.forward();
}

async function traverseHistoryFallback(tabId: number, direction: "back" | "forward", originalError: unknown): Promise<void> {
  const urlBefore = (await chrome.tabs.get(tabId)).url ?? "";
  try {
    await chrome.scripting.executeScript({
      target: { tabId },
      func: historyTraverse,
      args: [{ direction }],
    });
  } catch (err) {
    if (String(err).includes("Cannot access contents of the page")) {
      throw new BridgeError("NO_HISTORY", String(originalError));
    }
    throw err;
  }
  // history.back() without a previous entry is a silent no-op: require
  // evidence of the traversal (URL change or loading) or report NO_HISTORY
  // instead of a false success.
  const started = Date.now();
  for (;;) {
    const current = await chrome.tabs.get(tabId);
    if ((current.url ?? "") !== urlBefore || current.status === "loading") {
      return;
    }
    if (Date.now() - started >= HISTORY_FALLBACK_TIMEOUT_MS) {
      throw new BridgeError(
        "NO_HISTORY",
        `no traversable history entry for '${direction}' (native API error: ${String(originalError)})`,
      );
    }
    await sleep(HISTORY_FALLBACK_POLL_MS);
  }
}
...
async function navigate(params: Record<string, unknown>, tab?: chrome.tabs.Tab): Promise<unknown> {
  ...
  if (typeof url === "string" && url !== "") {
    ...
    if (action === "reload") {
      ...
      // Native API first; only the exact Chromium "not found" error arms the
      // history.back()/forward() fallback (any other error is rethrown).
      try {
        if (action === "back") await chrome.tabs.goBack(tab.id);
        else await chrome.tabs.goForward(tab.id);
      } catch (err) {
        if (!String(err).includes(NOT_FOUND_NEXT_PAGE_ERROR)) throw err;
        await traverseHistoryFallback(tab.id, action, err);
      }
    }
  } else {
